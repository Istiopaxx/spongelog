...
저는 모든 과정을 자동화하고 싶기 때문에 ChatGPT API를 활용하였습니다. 게다가 챗지피티 API는 질문과 답변을 하는데 있어서 더 조정할 수 있습니다.
...
그래서 간단히 챗지피티 웹페이지에서 가설 검증을 해보았는데, 생각보다 잘 해주더라구요.

챗지피티로 텍스트 데이터를 전처리할 때 포인트라고 느낀 부분은, 꽤나 상세히 요구사항을 명시해주어야 한다는 점입니다.
그리고 챗지피티 웹페이지에서보다 API가 더 프롬프트 엔지니어링을 할 여지가 많습니다.
